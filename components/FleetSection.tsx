...
import { Users, Star, Shield, ArrowRight, Car, CircleAlert } from "lucide-react";
...
export default function FleetSection() {
  ...
  useEffect(() => {
    fetch("/api/public/cars", { cache: "no-store" })
      .then((res) => (res.ok ? res.json() : []))
      .then((data) => setLiveCars(Array.isArray(data) ? data : []))
      .catch(() => setLiveCars([]))
      .finally(() => setLoading(false));
  }, []);

  const displayCars = liveCars.length > 0 ? liveCars : null;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="h-80 rounded-sm bg-white/5 animate-pulse reveal-up" />
            ))}
          </div>
        ) : displayCars ? (
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {displayCars.map((car, i) => (
                <div key={car._id} className="reveal-up group">
                  <div
                  ...
                  >
                    {/* Image */}
                    <div className="relative h-48 sm:h-52 md:h-56 overflow-hidden bg-[#0a1f14]">
                      {car.images && car.images.length > 0 ? (
                        <Image src={car.images[0]} alt={car.carName} fill className="object-cover transition-all duration-700 group-hover:scale-110" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
                      ) : (
                        <div className="flex h-full items-center justify-center"><Car size={64} className="text-white/10" /></div>
                      )}
                      <div className="absolute inset-0 bg-gradient-to-t from-[#050a08] via-[rgba(5,10,8,0.3)] to-transparent" />

                      {/* Top badge */}
                      <div className="absolute top-3 left-3">
                        <span className="text-[10px] font-body tracking-widest uppercase px-3 py-1.5 rounded-sm" style={{ color: "#c9a84c", background: "rgba(201,168,76,0.12)", border: "1px solid rgba(201,168,76,0.3)", backdropFilter: "blur(8px)" }}>
                          Available
                        </span>
                      </div>
                      ...
                      <div className="absolute bottom-3 right-3">
                        <span className="text-xs font-body font-semibold px-3 py-1 rounded-sm" style={{ background: "rgba(201,168,76,0.15)", border: "1px solid rgba(201,168,76,0.3)", color: "#f5e6b8" }}>
                    ...
                    <div className="p-5">
                      ...
                      <p className="text-white/50 text-sm font-body leading-relaxed mb-3" style={{ fontWeight: 300 }}>
                        {car.vehicleNumber}
                      </p>
                      ...
                      {car.features && car.features.length > 0 ? (
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {car.features.map((f, idx) => (
                            <span key={idx} className="text-[10px] font-body text-white/40 px-2.5 py-1 rounded-sm" style={{ border: "1px solid rgba(201,168,76,0.1)", background: "rgba(201,168,76,0.04)" }}>
                              {f}
                            </span>
                          ))}
                        </div>
                      ...
                      <p className="text-xs text-white/30 mb-4">Partner: {car.vendorName}</p>

                      {/* Divider */}
                      <div className="h-px w-full mb-4" style={{ background: "linear-gradient(90deg, transparent, rgba(201,168,76,0.2), transparent)" }} />
            ...
            <div className="reveal-up text-center mt-14">
              ...
              >
                <CircleAlert size={16} className="text-[#c9a84c]" />
                <span className="text-white/50 text-sm font-body">
                  {liveCars.length} vehicle{liveCars.length !== 1 ? "s" : ""} currently available &bull; Prices updated in real time
                </span>
              </div>
            </div>
          </>
              ...
              {fallbackCars.map((car, i) => (
                <div key={car.name} className="reveal-up group">
                  ...
                  >
                    <div className="relative h-48 sm:h-52 md:h-56 overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3 right-3 flex items-start justify-between">
                        <span
                          ...
                          style={{
                            color: car.badge === "Luxury" || car.badge === "Executive" ? "#f5e6b8" : "#c9a84c",
                            background: car.badge === "Luxury" || car.badge === "Executive" ? "rgba(245,228,154,0.12)" : "rgba(201,168,76,0.12)",
                            border: `1px solid ${car.badge === "Luxury" || car.badge === "Executive" ? "rgba(245,228,154,0.3)" : "rgba(201,168,76,0.3)"}`,
                        ...
                        >
                          ...
                          <span className="text-[11px] font-body text-white/80">{car.rating}</span>
                        </div>
                      </div>
                      <div className="absolute bottom-3 left-3">
                        <span className="text-[10px] font-body tracking-widest uppercase text-white/40">{car.tagline}</span>
                      </div>
                      <div className="absolute bottom-3 right-3">
                        <span
                    ...
                    <div className="p-5">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-display text-xl font-semibold text-white group-hover:text-[#c9a84c] transition-colors duration-300">{car.name}</h3>
                        <div className="flex items-center gap-1.5 text-white/40 text-xs font-body">
                          <Users size={12} className="text-[#c9a84c]" />
                          {car.capacity}
                        </div>
                      </div>
                      <p className="text-white/50 text-sm font-body leading-relaxed mb-4" style={{ fontWeight: 300 }}>{car.desc}</p>
                    ...
                    <div
                      className="absolute inset-0 pointer-events-none transition-opacity duration-500 opacity-0 group-hover:opacity-100"
                      style={{ border: "1px solid rgba(201,168,76,0.25)", boxShadow: "inset 0 0 40px rgba(201,168,76,0.04)" }}
                    />
                  </div>
                </div>
              ))}
